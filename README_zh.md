...
- [x] JIS（日语）布局符号，通过 `config/includes/jis.h`
- [ ] RGB，将在未来 v3 中支持
...
为使符号正确，键映射使用了 **`config/includes/jis.h`** 中定义的 `JP_*` 宏
（参考 QMK 的 `keymap_japanese.h`）。每个宏通过 JIS 驱动所期望的 HID 用法码 +
Shift 状态来输出目标字符。所有符号绑定以及 `mod_shift_*` mod-morph 行为都使用
这些宏。

注意：
- `` ` ``（反引号）为 `JP_GRV`（`LS(LBKT)`）；在 JIS 上直接使用 `&kp GRAVE`
  是 半角/全角（输入法切换）键，而非反引号。
- `]` / `}` 使用 `BSLH`（HID `0x31`）。如果你的主机将 `]}` 键映射为 `NUHS`
  （`0x32`），请将 `jis.h` 中的 `JP_RBRC` 改为 `NUHS`。
- 如果你想使用 **US** 布局，请将 PC 设为 US 并改回普通的 ZMK 符号键码
  （移除 `JP_*` 的使用 / `jis.h` 的 include）。
